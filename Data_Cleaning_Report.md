...
- Names, emails and cities were written inconsistently, like "jane doe" instead of "Jane Doe", or "JANE.DOE@EMAIL.COM" in all caps, plus some extra spaces like "Sarah Williams " with a trailing space
...
**Step 1 - Removed the duplicate row.** Customer 2's row was repeated exactly, so I used drop_duplicates() to remove the extra copy.

**Step 2 - Removed extra spaces.** Some entries had extra spaces before or after the text (like "Sarah Williams "), so I trimmed those using .strip().

**Step 3 - Fixed inconsistent capitalization.** I made all names and cities use Title Case, and made all emails lowercase, so everything looks consistent.

**Step 4 - Found and removed a hidden duplicate.** While checking the data I noticed David Lee (Customer 9) was actually entered twice under the same ID, just with one row missing the purchase amount. I kept the row that had the complete data and removed the other one.

**Step 5 - Fixed the date formats.** Since the dates were in 3 different formats, I converted all of them to one standard format: YYYY-MM-DD.

**Step 6 - Filled in missing values.** For missing names or dates, I couldn't guess the real value, so I filled them with "Unknown" or "Missing" instead of leaving them blank. For the missing purchase amounts, I filled them in using the median value of that column, since that's a safer choice than just using 0 or the average.

**Step 7 - Fixed the data types and saved the file.** I made sure CustomerID was stored as a whole number and PurchaseAmount was rounded to 2 decimal places, then saved the final result as cleaned_customers.csv.
